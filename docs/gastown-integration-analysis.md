# ccmux + Gas Town Integration Analysis

## Executive Summary

ccmux provides a viable replacement for tmux as Gas Town's session layer, offering:
...
The primary gap is exposing a few missing MCP tools (`ccmux_kill_session`, `ccmux_set_environment`) and adding a CLI compatibility layer for incremental migration.
...
| Gas Town Usage | tmux Command | ccmux Equivalent | Status | File:Line |
|----------------|--------------|------------------|--------|-----------|
| Create detached session | `new-session -d -s <name>` | `ccmux_create_session` | **IMPLEMENTED** | tools.rs:158-169 |
| Create session with command | `new-session -d -s <name> <cmd>` | `ccmux_create_session` (needs command param) | **PARTIAL** | CreateSession has optional command in protocol |
| Kill session | `kill-session -t <name>` | `DestroySession` in protocol | **NEEDS MCP TOOL** | messages.rs:149-150 |
| Check session exists | `has-session -t =<name>` | `ccmux_list_sessions` + filter | **EQUIVALENT** | tools.rs:137-143 |
| Kill server | `kill-server` | Not needed (ccmux daemon stays running) | **N/A** | |
| Rename session | `rename-session -t <old> <new>` | `ccmux_rename_session` | **IMPLEMENTED** | tools.rs:192-208 |
| Attach session | `attach-session -t <session>` | `AttachSession` in protocol | **NEEDS MCP TOOL** | messages.rs:88-89 |
| Switch client | `switch-client -t <session>` | `ccmux_focus_pane` (pane-level) | **EQUIVALENT** | tools.rs:123-135 |

### Message Sending (The Nudge Pattern)

| Gas Town Usage | tmux Command | ccmux Equivalent | Status | Notes |
|----------------|--------------|------------------|--------|-------|
| NudgeSession | `send-keys -t <sess> -l <text>` + `send-keys Enter` | `ccmux_send_input(submit: true)` | **IMPLEMENTED** | 500ms debounce not needed - direct IPC |
| NudgePane | `send-keys -t <pane> -l <text>` + Enter | `ccmux_send_input(pane_id, submit: true)` | **IMPLEMENTED** | |
| SendKeysRaw | `send-keys -t <sess> <keys>` | `ccmux_send_input(submit: false)` | **IMPLEMENTED** | |
| SendKeysReplace | `C-u` + `send-keys` | `ccmux_send_input` (need Ctrl-U sequence) | **MANUAL** | Send `\x15` before text |
| Accept bypass dialog | `send-keys Down Enter` | `ccmux_send_input` | **MANUAL** | Send key sequences |

**Key Insight**: ccmux eliminates the need for debounce hacks. The 500ms debounce in Gas Town exists because `tmux send-keys` is asynchronous and unreliable for large pastes. ccmux uses direct PTY writes via IPC - no debounce needed.

### Window/Pane Management

| Gas Town Usage | tmux Command | ccmux Equivalent | Status | File:Line |
|----------------|--------------|------------------|--------|-----------|
| Create window | `new-window -t <session>` | `ccmux_create_window` | **IMPLEMENTED** | tools.rs:171-190 |
| Create pane (split) | `split-window -t <window>` | `ccmux_create_pane` | **IMPLEMENTED** | tools.rs:42-70 |
| Select window | `select-window -t <session>:<idx>` | `ccmux_select_window` (bridge only) | **PARTIAL** | bridge.rs:738-751 |
...
| Close pane | `kill-pane` (implied) | `ccmux_close_pane` | **IMPLEMENTED** | tools.rs:109-121 |
| Focus pane | `select-pane -t <pane>` | `ccmux_focus_pane` | **IMPLEMENTED** | tools.rs:123-135 |

### Information Gathering

| Gas Town Usage | tmux Command | ccmux Equivalent | Status | File:Line |
|----------------|--------------|------------------|--------|-----------|
| List sessions | `list-sessions -F "#{...}"` | `ccmux_list_sessions` | **IMPLEMENTED** | tools.rs:137-143 |
| List panes | `list-panes -t <sess> -F "#{...}"` | `ccmux_list_panes` | **IMPLEMENTED** | tools.rs:11-22 |
| Capture pane | `capture-pane -p -t <sess> -S -N` | `ccmux_read_pane(lines: N)` | **IMPLEMENTED** | tools.rs:24-40 |
| Get pane command | `list-panes -F "#{pane_current_command}"` | `ccmux_get_status` | **IMPLEMENTED** | tools.rs:95-107 |
| Get pane ID | `list-panes -F "#{pane_id}"` | All panes return UUIDs | **IMPLEMENTED** | |
| Get pane workdir | `list-panes -F "#{pane_current_path}"` | `ccmux_get_status.cwd` | **IMPLEMENTED** | messages.rs:326 |
...
| Gas Town Usage | tmux Command | ccmux Equivalent | Status | Notes |
|----------------|--------------|------------------|--------|-------|
| IsAgentRunning | `list-panes -F "#{pane_current_command}"` + parsing | `ccmux_get_status.state` | **BETTER** | Native Claude detection |
...
| Gas Town Usage | tmux Command | ccmux Equivalent | Status | Notes |
|----------------|--------------|------------------|--------|-------|
| Status-left | `set-option -t <sess> status-left ...` | TUI renders its own status | **DIFFERENT** | ccmux client handles this |
| Status-right | `set-option status-right "#(gt status-line)"` | ccmux status bar built-in | **DIFFERENT** | |
...
|-----|--------|----------------------|----------|
| **ccmux_kill_session** | Can't stop workers | Low - protocol exists, just add MCP tool | Add to tools.rs |
| **ccmux_set_environment** | Can't set GT_RIG, BEADS_* | Medium - need to add to session struct | session.rs + new tool |
| **ccmux_get_environment** | Can't read env vars | Medium - paired with set | session.rs + new tool |
| **CLI wrapper** | Incremental migration | Medium - add `ccmux send-keys` compat | New binary |

### Medium Priority (Enhanced Integration)

| Gap | Impact | Implementation Effort | Notes |
|-----|--------|----------------------|-------|
| **ccmux_clear_history** | Can't clear scrollback on respawn | Low - already tracked, just truncate | |
| **ccmux_find_session_by_cwd** | Session discovery by workdir | Medium - need index | Useful for Gas Town patterns |
| **Session metadata** | Store agent identity per-session | Medium - extend SessionInfo | For GT_RIG, role, etc. |
| **ccmux_respawn_pane** | Hot reload pattern | Low - compose close + create | Could be convenience tool |

### Low Priority (Nice to Have)

| Gap | Impact | Notes |
|-----|--------|-------|
| `ccmux_display_message` | Status notifications | Could use orchestration instead |
| `ccmux_display_popup` | Mail viewer | Different UI paradigm |
| Custom key bindings | User convenience | ccmux client handles |

---

## 3. Novel Capabilities (ccmux Advantages)

### 3.1 Claude Cognitive State Detection

**File**: `ccmux-server/src/claude/detector.rs`

Gas Town's Witness patrol currently does blind polling - "is there output?" followed by arbitrary nudges. ccmux detects Claude's actual cognitive state:
...
**Integration Value**:
- Witness can query `ccmux_get_status(polecat-7).claude_state.activity`
...
**File**: `ccmux-protocol/src/messages.rs:8-67`

ccmux already has primitives matching Gas Town's messaging needs:
...
Instead of polling `tmux capture-pane` and parsing output, ccmux pushes:
...
**File**: `ccmux-server/src/persistence/`
...
**Integration Value**:
- `gt seance` can find Claude session IDs directly from ccmux state
- Workers survive daemon restarts with full context
- No more "orphan session" detection - ccmux tracks everything
...
ccmux doesn't need any of this:
...
Create `ccmux-compat` binary that mimics tmux commands:

```bash
# Gas Town calls:
ccmux-compat new-session -d -s gt-alpha-toast -c /repo claude

# Translates to:
ccmux_create_session(name: "gt-alpha-toast")
# + ccmux_send_input for initial command (or add command param)

# Gas Town calls:
ccmux-compat send-keys -t gt-alpha-toast -l "do your job"
ccmux-compat send-keys -t gt-alpha-toast Enter

# Translates to:
ccmux_send_input(pane_id: resolve("gt-alpha-toast"), input: "do your job", submit: true)
```

**One-line swap in Gas Town**: Change `exec.Command("tmux", args...)` to `exec.Command("ccmux-compat", args...)` in `internal/tmux/tmux.go:35`.
...
func (c *Client) NudgeSession(session, message string) error {
    ...
    return c.conn.Call("ccmux_send_input", map[string]any{
        "pane_id": paneId,
        "input": message,
        "submit": true,
    })
...
func (c *Client) GetClaudeState(session string) (*ClaudeState, error) {
    paneId := c.resolvePaneId(session)
    result := c.conn.Call("ccmux_get_status", map[string]any{
        "pane_id": paneId,
    })
...
Workers get MCP access to ccmux tools:

```
# Mayor spawns polecats directly
ccmux_create_session(name: "gt-alpha-new-polecat")

# Polecats check each other
ccmux_get_status(pane_id: "polecat-7").claude_state

# Witness queries all states
for pane in ccmux_list_panes(session: "gt-alpha-*"):
    if pane.claude_state.activity == "AwaitingConfirmation":
        ccmux_send_input(pane.id, "continue", submit: true)
...
### Goal
Prove ccmux can replace tmux for `gt nudge` without behavioral changes.

### Scope
1. Run Gas Town's mayor with ccmux instead of tmux
2. Nudge a polecat and verify message delivery
3. Check Claude state via MCP instead of parsing output

### Prerequisites
1. Add `ccmux_kill_session` MCP tool
2. Add `ccmux_set_environment` MCP tool
...
fn handle_send_keys(args: &[String]) {
    ...
    // Connect to ccmux daemon
    // Call ccmux_send_input
}
```
...
```go
// internal/integration/ccmux_test.go
func TestCcmuxNudge(t *testing.T) {
    // Start ccmux daemon
    // Create session via ccmux_create_session
    // Set GT_RIG via ccmux_set_environment
    // Start Claude in pane
    // Send nudge via ccmux_send_input
    // Verify response in ccmux_read_pane
    // Check ClaudeState via ccmux_get_status
    // Kill session via ccmux_kill_session
}
```

### Success Criteria
1. `gt nudge mayor "status"` works with ccmux backend
...
| Component | File | Key Lines |
|-----------|------|-----------|
| MCP tool definitions | `ccmux-server/src/mcp/tools.rs` | 1-268 |
| MCP bridge | `ccmux-server/src/mcp/bridge.rs` | 1-899 |
| MCP handlers | `ccmux-server/src/mcp/handlers.rs` | 1-1090 |
| Protocol messages | `ccmux-protocol/src/messages.rs` | 1-400 |
| Orchestration types | `ccmux-protocol/src/messages.rs` | 8-67 |
| Claude detector | `ccmux-server/src/claude/detector.rs` | 1-450+ |
| Session manager | `ccmux-server/src/session/manager.rs` | 1-200+ |
| Persistence | `ccmux-server/src/persistence/mod.rs` | 1-200+ |

---

## 7. Conclusion

ccmux is not just a tmux replacement - it's the "Orchestrator API Surface" that Yegge explicitly called for. The integration is viable with modest effort:
