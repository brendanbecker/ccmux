# ccmux Multi-Agent Orchestration Demo

You are an orchestrator running inside ccmux. Your job: demonstrate multi-agent workflows that showcase what makes ccmux unique - not just terminal multiplexing, but **AI agent coordination**.

## What Makes This Different

tmux can split terminals. ccmux can:
...
- Synchronize on output patterns with `ccmux_expect`
...
Say: "I'm an orchestrator agent running in ccmux. Watch me coordinate a team of AI workers to accomplish parallel tasks - something no terminal multiplexer has done before."
...
```
ccmux_create_session(name: "worker-alpha", cwd: "/home/becker/projects/tools/ccmux")
ccmux_create_session(name: "worker-beta", cwd: "/home/becker/projects/tools/ccmux")
ccmux_create_session(name: "worker-gamma", cwd: "/home/becker/projects/tools/ccmux")
```

3. Tag sessions for routing:
```
ccmux_set_tags(session: "worker-alpha", add: ["worker", "rust"])
ccmux_set_tags(session: "worker-beta", add: ["worker", "docs"])
ccmux_set_tags(session: "worker-gamma", add: ["worker", "test"])
```
...
```
ccmux_send_input(pane_id: <worker-alpha-pane>, input: "claude\n")
ccmux_send_input(pane_id: <worker-beta-pane>, input: "claude\n")
ccmux_send_input(pane_id: <worker-gamma-pane>, input: "claude\n")
```

3. Wait for Claude to start using expect:
```
ccmux_expect(pane_id: <worker-alpha-pane>, pattern: "Claude Code", timeout_ms: 30000)
ccmux_expect(pane_id: <worker-beta-pane>, pattern: "Claude Code", timeout_ms: 30000)
ccmux_expect(pane_id: <worker-gamma-pane>, pattern: "Claude Code", timeout_ms: 30000)
```

4. Verify agent detection:
```
ccmux_list_panes()  # Should show is_claude: true for worker panes
```

5. Say: "All three Claude instances detected and ready. ccmux knows these aren't just shells - they're AI agents."

### Act 4: Parallel Task Distribution (60 seconds)

1. Say: "Let's distribute work. I'll use ccmux_run_parallel to execute commands across multiple panes simultaneously."
...
```
ccmux_get_status(pane_id: <each worker pane>)
```
...
1. Say: "ccmux integrates with beads for issue tracking. Let me assign work items to workers."

2. Assign issues to workers:
```
ccmux_beads_assign(pane_id: <worker-alpha-pane>, issue_id: "FEAT-097")
ccmux_beads_assign(pane_id: <worker-beta-pane>, issue_id: "BUG-047")
```

3. Check assignments:
```
ccmux_beads_find_pane(issue_id: "FEAT-097")  # Should return worker-alpha
```
...
```
ccmux_report_status(status: "working", message: "Coordinating worker pool")
```
...
```
ccmux_mirror_pane(source_pane_id: <worker-alpha-pane>)
```
...
1. Say: "For dependent operations, ccmux_run_pipeline executes commands in sequence."
...
```
ccmux_beads_release(pane_id: <worker-alpha-pane>, outcome: "completed")
```

2. Check worker history:
```
ccmux_beads_pane_history(pane_id: <worker-alpha-pane>)
```

3. Kill worker sessions:
```
ccmux_kill_session(session: "worker-alpha")
ccmux_kill_session(session: "worker-beta")
ccmux_kill_session(session: "worker-gamma")
```
...
5. Say: "This is ccmux - not a terminal multiplexer with AI bolted on, but an **AI orchestration platform** that happens to manage terminals."

## Key Tools Showcased

| Tool | Purpose |
|------|---------|
| `ccmux_create_session` | Spawn isolated workspaces |
| `ccmux_set_tags` | Tag-based message routing |
| `ccmux_send_orchestration` | Inter-agent communication |
| `ccmux_report_status` | Worker status updates |
| `ccmux_expect` | Pattern-based synchronization |
| `ccmux_run_parallel` | Parallel command execution |
| `ccmux_run_pipeline` | Sequential command pipelines |
| `ccmux_mirror_pane` | Real-time output monitoring |
| `ccmux_beads_assign` | Work item tracking |
| `ccmux_list_panes` | Agent state detection |
