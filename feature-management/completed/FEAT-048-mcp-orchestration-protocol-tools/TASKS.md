...
- [ ] Review existing orchestration types in ccmux-protocol/src/messages.rs
- [ ] Review existing MCP tools in ccmux-server/src/mcp/tools.rs

## Phase 1: Core Infrastructure

- [ ] Add CCMUX_SESSION_ID environment variable to PTY spawn in ccmux-server/src/pty/spawn.rs
- [ ] Create OrchestrationTarget JSON schema
- [ ] Create OrchestrationMessage JSON schema
- [ ] Implement `ccmux_send_orchestration` tool handler
...
- [ ] Implement session context resolution from CCMUX_SESSION_ID
- [ ] Implement `ccmux_report_status` tool
  - [ ] Tool schema
  - [ ] Handler with auto-fill session_id
  - [ ] Unit tests
- [ ] Implement `ccmux_request_help` tool
  - [ ] Tool schema
  - [ ] Handler with auto-target to Orchestrator
  - [ ] Unit tests
- [ ] Implement `ccmux_broadcast` tool
  - [ ] Tool schema
...
- [ ] Implement `ccmux_get_orchestration_messages` polling tool
- [ ] Add queue size limits and cleanup
- [ ] Integration test for message receive flow

## Testing Tasks

- [ ] Test all OrchestrationMessage variants via ccmux_send_orchestration
...
- [ ] Document tool schemas in ccmux-server/src/mcp/README.md (or equivalent)
