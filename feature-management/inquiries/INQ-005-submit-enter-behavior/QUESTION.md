# INQ-005: Submit/Enter Behavior in ccmux Input Tools

## Problem Statement

During orchestration setup, we observed that Enter key presses may not be reliably sent through ccmux input mechanisms:

1. **`ccmux_send_input` with `submit: true`** - When sending a prompt to a Claude agent pane with `submit: true`, the Enter may not be sent, leaving the message in the input buffer unsubmitted.
...
- Manual `ccmux_send_input` with `key: "Enter"` was required to submit

## Research Questions

### Area 1: `ccmux_send_input` Submit Parameter
