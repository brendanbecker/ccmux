...
Add a `{"target": {"parent": true}}` option to `ccmux_send_orchestration` that automatically resolves the sender's parent session using the `child:<parent-name>` lineage tag pattern.

## Problem Statement

Currently, `ccmux_report_status` and `ccmux_request_help` hardcode the target as `OrchestrationTarget::Tagged("orchestrator")`. This causes problems in multi-orchestrator scenarios:
...
`ccmux_report_status` and `ccmux_request_help` should use Parent target by default instead of broadcasting to all orchestrators.
...
Update the `ccmux_send_orchestration` target schema to include parent option:
...
**File**: `ccmux-server/src/session/manager.rs` (or wherever orchestration routing happens)
...
| File | Changes |
|------|---------|
| `ccmux-protocol/src/messages.rs` | Add `Parent` variant to `OrchestrationTarget` |
| `ccmux-server/src/mcp/tools.rs` | Update tool schema for parent target |
| `ccmux-server/src/mcp/bridge/handlers.rs` | Parse parent target, update report_status/request_help |
| `ccmux-server/src/session/manager.rs` | Handle Parent target resolution |
...
- [ ] Update `ccmux_send_orchestration` schema with parent option
...
- [ ] `ccmux_report_status` uses Parent target by default
- [ ] `ccmux_request_help` uses Parent target by default
...
For urgent notifications (like "worker stuck"), the watchdog could use `ccmux_send_input` directly to inject messages into the orchestrator's terminal. This is complementary - `send_input` for interrupts, `send_orchestration` with Parent for structured messages.
