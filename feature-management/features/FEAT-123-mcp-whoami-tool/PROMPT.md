# FEAT-123: Add ccmux_whoami MCP tool for agent self-identification
...
Agents running inside ccmux panes cannot reliably determine their own identity. When `ccmux_get_tags` is called without a session filter, it returns an arbitrary session (e.g., "obsidian") instead of the session the calling agent is actually in (e.g., "ccmux-orch").
...
The MCP bridge runs as a subprocess of Claude Code, which runs inside a ccmux pane. The pane has `CCMUX_PANE_ID` set in its environment (FEAT-053), but the MCP bridge doesn't use this to establish context. Instead, `ccmux_get_tags` without a filter just returns the first session it finds.

## Solution

Add a `ccmux_whoami` tool that:
1. Reads `CCMUX_PANE_ID` from the MCP server process's environment
2. Looks up the pane to find its session
3. Returns complete identity information

## Proposed Tool

### Tool Name: `ccmux_whoami`
...
```json
{
  ...
  "session_name": "ccmux-orch",
  "window_id": "4de6333e-3c84-4d0f-97d3-0b8b4d3bb99c",
  "tags": ["orchestrator", "ccmux", "child:main-orch"],
  "cwd": "/home/user/projects/ccmux"
}
```

### Error Cases

If `CCMUX_PANE_ID` is not set (running outside ccmux):
```json
{
  "error": "Not running inside ccmux",
  "detail": "CCMUX_PANE_ID environment variable not set"
}
```
...
{
    "name": "ccmux_whoami",
    "description": "Get the identity of the current pane/session. Returns pane ID, session ID, session name, tags, and cwd. Uses CCMUX_PANE_ID environment variable.",
    "inputSchema": {
        "type": "object",
        "properties": {
            "include_tags": {
...
pub async fn tool_whoami(&mut self, include_tags: bool) -> Result<ToolResult, McpError> {
    ...
    let pane_id_str = std::env::var("CCMUX_PANE_ID")
        .map_err(|_| McpError::InvalidParams(
            "Not running inside ccmux: CCMUX_PANE_ID not set".into()
        ))?;

    let pane_id = Uuid::parse_str(&pane_id_str)
        .map_err(|e| McpError::InvalidParams(
            format!("Invalid CCMUX_PANE_ID: {}", e)
        ))?;
...
**File**: `ccmux-server/src/session/manager.rs`
...
| `ccmux-server/src/mcp/bridge/mod.rs` | Route tool call |
| `ccmux-server/src/mcp/bridge/handlers.rs` | Implement handler |
| `ccmux-protocol/src/messages.rs` | Add GetPaneInfo/PaneInfo messages |
| `ccmux-server/src/session/manager.rs` | Handle GetPaneInfo |
...
### Section 3: Testing
- [ ] Test inside ccmux pane - returns correct identity
- [ ] Test outside ccmux - returns appropriate error
- [ ] Test with include_tags=false

## Acceptance Criteria

- [ ] `ccmux_whoami` returns correct pane_id, session_id, session_name
- [ ] Tags included by default, optional to exclude
- [ ] Graceful error when not running inside ccmux
...
```
Orchestrator calls ccmux_whoami -> session_name = "orch-main"
...
- FEAT-053: Auto-inject ccmux context env vars (provides CCMUX_PANE_ID)
- FEAT-122: Parent target for orchestration (uses lineage tags)
- BUG: ccmux_get_tags returns wrong session without filter
