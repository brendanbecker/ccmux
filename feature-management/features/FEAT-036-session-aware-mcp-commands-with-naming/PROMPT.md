...
The ccmux MCP tools (like `ccmux_create_pane`) currently default to the wrong session when multiple sessions exist. When a user calls `create_pane` without specifying a session, it picks the first session in the list rather than the session that has an attached client.

### Current Behavior

- `ccmux_list_windows` says "uses first session if omitted"
- `ccmux_create_window` says "uses first session if omitted"
- `ccmux_create_pane` creates pane "in the current session" but doesn't define what "current" means
...
Calling `ccmux_create_pane` without specifying a session created the pane in Session A (the orphaned session) instead of Session B (where the user is working).
...
|------|-----------------|-------------|
| `ccmux_list_windows` | First session | Active session (with attached clients) |
| `ccmux_create_window` | First session | Active session |
| `ccmux_create_pane` | Ambiguous "current session" | Active session |
| `ccmux_list_panes` | Optional session filter | Active session if filter omitted |
...
**`ccmux_create_pane`** - Add `name` parameter:
```json
{
  "name": {
    "type": "string",
...
**`ccmux_create_window`** - Already has `name` parameter (verify it works)
...
**`ccmux_list_panes`** response should include name:
```json
{
  "id": "uuid",
...
**`ccmux_list_windows`** response should include name:
```json
{
  "id": "uuid",
...
| File | Changes |
|------|---------|
| `ccmux-server/src/mcp/tools.rs` | Add name param to create_pane, add rename tools |
| `ccmux-server/src/mcp/handlers.rs` | Implement active session logic, rename handlers |
| `ccmux-server/src/mcp/server.rs` | Add routing for rename tools |
| `ccmux-server/src/session/mod.rs` | Add attached_clients tracking, name field to Pane |
| `ccmux-server/src/session/window.rs` | Add name field to Pane if not present |
| `ccmux-server/src/session/pane.rs` | Add name field if not present |
...
- [ ] Update `ccmux_list_windows` to use active session
- [ ] Update `ccmux_create_window` to use active session
- [ ] Update `ccmux_create_pane` to use active session
- [ ] Update `ccmux_list_panes` to use active session when no filter
...
- [ ] Add `name` parameter to `ccmux_create_pane`
- [ ] Verify `ccmux_create_window` name parameter works
- [ ] Implement `ccmux_rename_pane` tool
- [ ] Implement `ccmux_rename_window` tool
- [ ] Update `ccmux_list_panes` to include name in output
- [ ] Update `ccmux_list_windows` to include name in output
...
- [ ] `ccmux_list_panes` shows pane names
- [ ] `ccmux_list_windows` shows window names
...
**Call**: `ccmux_create_pane` with no session parameter
...
**Expected**: Pane created with name "claude_main" visible in `ccmux_list_panes`
