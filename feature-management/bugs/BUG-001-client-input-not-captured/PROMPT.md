...
1. Start server: `cargo run --bin ccmux-server`
2. Start client: `cargo run --bin ccmux`
...
```
2026-01-09T16:06:58.477663Z  INFO ccmux_server: Client connected, active clients: 1
2026-01-09T16:06:58.477853Z  INFO ccmux_server: Client e0c65e44-1bf5-4d70-b2a4-64245c74322b connecting with protocol version 1
```

## Investigation Findings (2026-01-09)

### Pre-existing Bug
This bug existed before FEAT-021 but was masked because the server never ran. FEAT-021 only changed `ccmux-utils/src/lib.rs` (added a re-export), not client code.

### Trace Logging Results
Running with `CCMUX_LOG=trace` shows:
```
2026-01-09T16:19:41.599785Z  INFO ccmux: ccmux client starting
...
### Root Cause Analysis
The input polling thread (`ccmux-client/src/ui/event.rs:69`) uses `std::thread::spawn` with `crossterm::event::poll()` and sends to a tokio mpsc channel. The thread is either:
...
| File | Line | Function |
|------|------|----------|
| `ccmux-client/src/ui/event.rs` | 65-115 | `start_input_polling()` - spawns input thread |
| `ccmux-client/src/ui/event.rs` | 72 | `event::poll(tick_rate)` - may be blocking |
| `ccmux-client/src/ui/app.rs` | 130 | `events.next().await` - blocks waiting for events |
| `ccmux-client/src/ui/terminal.rs` | 27 | `enable_raw_mode()` - terminal setup |
