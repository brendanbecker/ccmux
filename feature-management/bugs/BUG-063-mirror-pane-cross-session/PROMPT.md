...
The whole point of mirror panes is "plate spinning" - an orchestrator in session A should be able to create mirror panes to watch agents running in sessions B, C, D, etc. Currently, if you call `ccmux_mirror_pane` from session A targeting a pane in session B, the mirror is created in session B - which is useless.

## Reproduction Steps

1. Have orchestrator in `session-0`
2. Have worker agent in `worker-session` with pane `worker-pane-id`
3. From orchestrator, call `ccmux_mirror_pane(source_pane_id: "worker-pane-id")`
...
Looking at `ccmux-server/src/handlers/pane.rs` `handle_create_mirror`:
...
The MCP tool `ccmux_mirror_pane` should:
...
- [ ] `ccmux_mirror_pane` creates mirror in the caller's session by default
