...
$(cat "$tasks_file")"
    ...
    if $DRY_RUN; then
        ...
        # Extract bead ID from result (format: "✓ Created issue: xx-xxx")
        local bead_id=$(echo "$result" | grep -oP '[a-z]{2}-[a-z0-9]+' | head -1)
...
migrate_bug() {
    ...
    # Extract metadata from JSON (handle both 'bug_id' and 'id' field names)
    local bug_id=$(jq -r '.bug_id // .id // empty' "$json_file")
    local title=$(jq -r '.title // empty' "$json_file")
    local priority=$(jq -r '.priority // "P2"' "$json_file")
